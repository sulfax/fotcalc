...
const kamper = [
    ...
    ['Odd',190208],
    ['RBK',219041,220406,220997,221059,221065],
    ['Sandefjord',175224,226577],
    ['Sarpsborg',174890,174891,174893],
    ['Strømsgodset',225217],
    ['Tromsø',190898,190899,190900],
    ['Viking',223382,215906,223383,223386],
    ['Vålerenga',194301,194302,194303],

    ['RBK Kvinner',222885,226547],
...
function generer_kalender(klubb) {
    ...
    for (h = 0; h < kamper.length; h++) {
        if (kamper[h][0] == klubb) {
            ...
            request.open("GET", 'https://cors-anywhere.herokuapp.com/https://ticketco.events/no/nb/events/' + kamper[h][p] + '/seating_arrangement/', true);
            request.send(null);
            var kamptittel = "";
            f = h;
